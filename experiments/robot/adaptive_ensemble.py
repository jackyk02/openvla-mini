...
class AdaptiveEnsembler:
    ...
    def fake_ensemble_action(self, cur_action):
        fake_action_history = self.action_history.copy()
        fake_action_history.append(cur_action)
        num_actions = len(fake_action_history)
        if cur_action.ndim == 1:
            curr_act_preds = np.stack(fake_action_history)
        else:
            curr_act_preds = np.stack(
                [pred_actions[i] for (i, pred_actions) in zip(range(num_actions - 1, -1, -1), fake_action_history)]
            )
    ...
    def append_to_history(self, cur_action):
